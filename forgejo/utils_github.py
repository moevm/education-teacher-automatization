...
def create_repo_with_settings_github(user_object, name, is_private=False, create_readme=False, template=False,
...
def give_an_access_github(repo_object, collaborator, pull=False, admin=False):
...
def remove_an_access_github(repo_object, collaborator):
...
def change_an_access_github(users, user_object, repo, give, pull=False, admin=False):
    repo_object = create_repo_with_settings_github(user_object, name=repo)
    ...
    for user in users:
        if give:
            give_an_access_github(repo_object, user, pull, admin)
        else:
            remove_an_access_github(repo_object, user)
...
def token_probe_reqest_github(access_token):
...
def get_token_from_file_github(token_path):
    try:
...
def auth_github(token_path):
    token = get_token_from_file_github(token_path)

    token_status = token_probe_reqest_github(token)
