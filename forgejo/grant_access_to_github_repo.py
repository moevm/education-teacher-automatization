...
from github_utils import auth_github, change_an_access_github, get_user_object
from forgejo_utils import auth_forgejo, create_repo_with_settings_forgejo, change_an_access_forgejo
...
def processing_access_list_github(github_object, access_list, giving, pull=False, admin=False):
    for obj in access_list:
        sleep(0.05)
        repo_link, users = obj.split(':', 1)
        print(repo_link, pull)
        repo_owner, repo = repo_link.split('/', 1)
        print('Owner: {}, repo: {}'.format(repo_owner, repo))
        user_object = get_user_object(github_object, repo_owner)
        sleep(0.05)
        change_an_access_github(users.split(','), user_object, repo, giving, pull, admin)

def process_access_list_forgejo(session, access_list, give_access, pull=False, admin=False):
    for item in access_list:
        ...
        repo_info = create_repo_with_settings_forgejo(session, owner, repo)
        if not repo_info:
            continue
        
        change_an_access_forgejo(session, users.split(','), 
                         owner, repo,give_access,
                         pull, admin)
...
if __name__ == "__main__":
    ...
    if args.forgejo:
        ...
        if args.give_access:
            process_access_list_forgejo(session, args.give_access, True, args.p, args.a)
        
        if args.remove_access:
            process_access_list_forgejo(session, args.remove_access, False)
    else:
        ...
        admin = args.a
        processing_access_list_github(github_object, give_an_access_list, True, pull, admin)
        processing_access_list_github(github_object, remove_an_access_list, False)